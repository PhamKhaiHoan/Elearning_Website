...
import { Users, BookOpen, LayoutDashboard, LogOut, Home } from "lucide-react";
...
const LayoutQuanTri = () => {
  ...
  const danhSachMenu = [
    ...
    { ten: "Người Dùng", duongDan: "/admin/users", icon: <Users size={20} /> },
    {
      ten: "Khóa Học",
    ...
    <div className="flex h-screen bg-gray-100">
      <div className="w-64 bg-white shadow-lg flex flex-col justify-between">
        ...
        <div className="p-4 border-t border-gray-200 space-y-2">
          <Link
            to="/"
            className="flex items-center gap-2 text-blue-500 hover:text-blue-700 w-full px-4 py-2"
          >
            <Home size={20} />
        ...
        <header className="h-16 bg-white shadow-sm flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-500">
              Xin chào, {nguoiDung?.hoTen || "Quản trị viên"}
            </span>
            <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">
              {(nguoiDung?.hoTen?.[0] || "A").toUpperCase()}
            </div>
          </div>
        </header>
